...
void sql_pool(http_conn *users){
    m_connPool = mysql_conn::GetInstance();
    m_connPool->init("127.0.0.1","root", "123456", "test", 3306, 8);
...
int main()
{
    ...
    //创建epoll对象，事件数组，添加
    epoll_event events[MAX_ENENT_NUMBER];
    ...
    while (1)
    {
        int num = epoll_wait(epollfd, events, MAX_ENENT_NUMBER, -1);
